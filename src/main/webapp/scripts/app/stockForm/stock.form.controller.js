...
angular.module('cabzzaApp')
		.controller('StockFormController', function ($scope, $state, $http, StockWallet, Account, filterFilter, $rootScope,
...
//			}, true);
			...
			$scope.quote = {};
			$scope.refreshQuotes = function (query) {
				return $http.get(
						'/lookup?query=' + query
						).then(function (response) {
					$scope.quotes = response.data.ResultSet.Result
				});
			};
			$scope.add = function () {
			...
			$scope.atLeastOne = function () {
				return $scope.selectedQuotes.length > 0;
			}
